...
- Issueとの関係（Issue詳細は backlog/ に置く）
...
- Security gate: `docs/10_SECURITY.md` の該当要件を満たす
- Observability gate: 主要シナリオの観測項目/閾値を定義し、`docs/07_RUNBOOK.md` に反映する
- Operability gate: デプロイ/ロールバック/Secrets変更手順を `docs/07_RUNBOOK.md` に反映する
...
- [ ] docs/ の骨格が揃っている（00_PRD〜10_SECURITY、DEVFLOW、README）
- [ ] AGENTS.md がある（DoD定義）
- [ ] CIが最低限回る開発環境構築
- [ ] `docs/10_SECURITY.md` に初版のセキュリティ要件が定義されている
...
### Notes
- このMilestonesは「最初に整えるべき契約」を固めるフェーズ。docs/ の骨格（00_PRD〜10_SECURITY、DEVFLOW、README）とバックログ運用（ISSUE-template 等）を揃える。
